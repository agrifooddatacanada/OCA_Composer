...
export class OCAParser {
  ...
  static parseSchemaData(schemaId, ocaPackage) {
    ...
    const captureBaseId = OCAParser._resolveCaptureBaseId(
    ...
    if (
      schemaId === bundleId ||
      schemaId === flatOcaPackageForParsing.bundle?.capture_base?.d ||
      schemaId === "root"
    ) {
      sensitiveAttributeNames =
        flatOcaPackageForParsing.bundle?.capture_base?.flagged_attributes || [];
    ...
    const adcSensitiveAttrs = Array.isArray(sensitiveOverlay?.sensitive_attributes)
      ...
      : [];
    sensitiveAttributeNames = [
      ...new Set([...sensitiveAttributeNames, ...adcSensitiveAttrs])
    ];
    ...
    if (Array.isArray(orderingNamesRaw) && orderingNamesRaw.length > 0) {
      ...
      attributesOrdered = OCAParser._applyAttributeOrder(
        attributesWithLists,
        orderingNames
      );
      lanOrdered = OCAParser._orderLanRowsForAttributes(
        lanAttributeRowData,
        attributesOrdered
      );
    }
    ...
    if (firstLang && lanOrdered[firstLang]) {
      attributesOrdered.forEach((attr) => {
        const lanData = lanOrdered[firstLang].find(
          (lan) => lan.Attribute === attr.Attribute
        );
    ...
    this._processConformanceOverlay(schemaData.overlays?.conformance, attributesOrdered);

    // Process unit overlay to populate Unit field in attributes
    this._processUnitOverlay(schemaData.overlays?.unit, attributesOrdered);
  ...
  static _applyAttributeOrder(attributesWithLists, orderingNames) {
    if (!Array.isArray(orderingNames) || orderingNames.length === 0)
      return attributesWithLists;
  ...
  static _parseEntryOverlays(entryOverlay) {
    ...
    const addEntry = (attrName, code, langCode, text) => {
      ...
      const langName =
        langNameFromCodeOCA(langCode) || langNameFromTwoLetters(langCode) || langCode;
      row[langName] = text || "";
    };
  ...
  static _parseLabelOverlays(labelOverlays, informationOverlays, attributesWithLists) {
    ...
    const allLanguages = new Set([
      ...Object.keys(labelsByLang),
      ...Object.keys(descriptionsByLang)
    ]);
  ...
  static _parseOverlayData(
    ...
    const unitFramingExtension =
      ocaPackage?.extensions?.adc?.[schemaId]?.overlays?.unit_framing;
  ...
  static _parseFormOverlay(ocaPackage, schemaId) {
    ...
    const formOverlayData =
      ocaPackage.extensions.adc[schemaId].overlays.form_overlay ||
    ...
    formOverlayArray.forEach((overlay) => {
      ...
      Object.entries(overlay.interaction[0].arguments).forEach(([attr, argData]) => {
        if (argData?.placeholder) {
          const { placeholder } = argData;

          if (typeof placeholder === "string") {
            formPlaceholdersByLanguage[langName][attr] =
              normalizeEscapedQuotes(placeholder);
            ...
            if (placeholderValue) {
              formPlaceholdersByLanguage[langName][attr] =
                normalizeEscapedQuotes(placeholderValue);
            }
          }
        }
      });
   ...
   */
  static _parseFormOverlayStructure(
    ocaPackage,
    schemaId,
    attributes,
    overlays,
   ...
   * @private
   */
  static _buildOverlaySelections(
    overlays,
    hasUnitFramingExtension = false,
    hasRangeExtension = false,
    hasFormExtension = false,
    hasDataSeparatorExtension = false
  ) {
    const charEncodingOverlay = overlays?.character_encoding;
    const formatOverlay = overlays?.format;
    const cardinalityOverlay = overlays?.cardinality;
    ...
    const selections = {
      [FIELD_CHARACTER_ENCODING_OVERLAY]:
        !!charEncodingOverlay?.attribute_character_encoding,
  ...
  static _parseDataSeparatorOverlays(adcExtensions) {
    ...
    if (Array.isArray(adcExtensions)) {
      decimalOverlay = adcExtensions.find(
        (ov) => ov?.decimal_separator_overlay
      )?.decimal_separator_overlay;
      fileOverlay = adcExtensions.find(
        (ov) => ov?.file_delimiter_overlay
      )?.file_delimiter_overlay;
      arrayOverlay = adcExtensions.find(
        (ov) => ov?.array_delimiter_overlay
    ...
    if (fileOverlay) {
      result.fileDelimiterData = {
        ...
        dataStartRow:
          typeof fileOverlay.data_start_row === "number"
