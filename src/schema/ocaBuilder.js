...
function schemaHasEdits(schemaState) {
  return (
    ...
    (schemaState?.attributes && schemaState.attributes.length > 0)
  );
}
...
function findChildSchemaStateEntry(schemaStates, ...logicalNames) {
  ...
  const names = [
    ...new Set(logicalNames.filter((n) => n !== undefined && n !== null && n !== ""))
  ];
 ...
 *
 * @param {Object|null} ocaPackage - Original OCA package JSON structure (null for manual creation)
 ...
 */
export function buildOcaPackageJsonFromEditorState({
  ocaPackage,
  schemaStates,
  getSchemaById
}) {
  ...
  if (!ocaPackage) {
    ...
    pkg = JSON.parse(JSON.stringify(ocaPackage));
 ...
 *
 */
function applySchemaStateToPkg({
  pkg,
  schemaId,
  schemaState,
  getSchemaById,
...
export function findOrCreatePkgSchema({
  ...
  if (!parentSchemaId) {
    for (const [stateSchemaId] of Object.entries(schemaStates)) {
      ...
      const hasRefn = schemaState.attributes.some(
        (attr) =>
          attr.Attribute === schemaId &&
  ...
  const newDependency = createMinimalOCASchema(schemaId, {
    captureBaseId,
    overlays: {
      meta: [createMetaOverlay(captureBaseId, "eng", displayName)]
    }
  });
 ...
 */
export function rebuildAttributes(
  schema,
  schemaState,
  schemaStates = {},
  getSchemaById = null
  ...
  schemaState.attributes.forEach((attr) => {
    ...
    // Second priority: Check originalAttributes from package (for schemas that were edited)
      ...
      if (
        ...
        } else if (originalValue.startsWith("refn:")) {
 ...
 */
export function ensureChildSchemaDependencies(pkg, schemaStates, getSchemaById) {
  ...
  while (foundNewDependencies && iteration < maxIterations) {
    ...
    Object.keys(schemaStates).forEach((schemaId) => {
      ...
      if (schemaState.attributes) {
        schemaState.attributes.forEach((attr) => {
          if (
            attr.Type === TYPE_CHILD_SCHEMA ||
            attr.Type === TYPE_PLACEHOLDER_CHILD_SCHEMA
          ) {
            ...
            if (
              schemaHasEdits(childschemaState) &&
    ...
    allSchemas.forEach((schema) => {
      ...
      Object.entries(attributes).forEach(([key, value]) => {
        ...
        // Handle refs: references from original package - always preserve child schemas with attributes
          ...
          if (
            schemaHasEdits(refsSchemaState) &&
            refsSchemaState.attributes &&
            refsSchemaState.attributes.length > 0
          ) {
    ...
    for (const [childSchemaName, childschemaState] of childSchemasToCreate) {
      ...
      childschemaState.attributes?.forEach((childAttr) => {
        if (childAttr.Attribute) {
          ...
          if (
            childAttr.Type === TYPE_CHILD_SCHEMA ||
            childAttr.Type === TYPE_PLACEHOLDER_CHILD_SCHEMA
          ) {
      ...
      if (!newDependency.overlays.meta || newDependency.overlays.meta.length === 0) {
        ...
        const displayName = resolveChildSchemaDisplayName(
          childSchemaName,
          bundle,
          dependencies
        );
...
export function ensurePlaceholderDependencies(pkg) {
  ...
  const extractPlaceholderId = (attrType) => {
    if (typeof attrType === "string" && attrType.startsWith("refn:")) {
      ...
      return refValue.startsWith("placeholder_")
        ? refValue.replace("placeholder_", "")
        : refValue;
    }
    return null;
  };
  ...
  if (bundle?.capture_base?.attributes) {
    Object.entries(bundle.capture_base.attributes).forEach(([, attrType]) => {
  ...
  (dependencies || []).forEach((dep) => {
    if (dep?.capture_base?.attributes) {
      Object.entries(dep.capture_base.attributes).forEach(([, attrType]) => {
        const placeholderId = extractPlaceholderId(attrType);
        if (placeholderId && !existingDepIds.has(placeholderId)) {
          placeholdersToCreate.add(placeholderId);
        }
...
function findDependencyByRefId(dependencies, id) {
  ...
  const byId = dependencies.find((d) => d && (d.d === id || d.capture_base?.d === id));
  if (byId) return byId;
  return (
    dependencies.find((d) => {
