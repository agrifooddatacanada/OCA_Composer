...
export function generateDataEntry(acceptedFiles, setLoading, selectedLang) {
  ...
  function readJSON(originJsonData_jsonSaid, e) {
    try {
      ...
      let json = rawJson?.["schema"]?.[0]
        ...
        ? rawJson?.["bundle"]
        : rawJson;
      originJsonData_jsonSaid.jsonSaid = json.d; // jsonSaid
  ...
  async function readZIP(originJsonData_jsonSaid, e) {
    ...
    originJsonData_jsonSaid.jsonSaid = "unavailable"; // jsonSaid
    return originJsonData_jsonSaid;
  }
  ...
  try {
    ...
    reader.onload = async (e) => {
      ...
      try {
        readJSON(originJsonData_jsonSaid, e);
      } catch (jsonError) {
        try {
          await readZIP(originJsonData_jsonSaid, e);
        } catch (zipError) {
      ...
      */
      ...
      try {
        schemaName = metaOverlays[0].name;
      ...
      sheet1.getCell(
        9,
        2
      ).value = `schema classification: ${schemaClassification}`;
      ...
      sheet1.getCell(15, 3).value =
        "The' Data Entry' sheet is where a user enters data.";
          ...
          Object.entries(overlay.attributes).forEach(
            ([attrName, attrType], index) => {
              ...
              if (attrIndex !== undefined) {
                ...
                throw new WorkbookError(
                  ".. Error check the attribute name ..."
                );
              }
              ...
              if (attrIndex !== undefined) {
                ...
                throw new WorkbookError(
                  ".. Error check the attribute type ..."
                );
              }
        ...
        if (overlay.type && overlay.type.includes("/character_encoding/")) {
          ...
          try {
            ...
            for (let [attrName, format] of Object.entries(
              ...
              const attrTypeFromAttrKeys = attrKeys.map((key) =>
                key.split(",")
              );
              const attrTypeObjects = attrTypeFromAttrKeys.map(
                ([attr, type]) => ({ attr, type })
              );
