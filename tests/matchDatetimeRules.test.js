...
test("matchDatetime (ISO: YYYYMMDD: year month day)", () => {
  ...
  const testDates = ["00000101", "99991231", "20230229", "20240229", "20230431"];

  const badTestDates = ["100000232", "20231331", "20231631", "1050931", "202340230"];
...
test("matchDatetime (ISO: YYYY-MM: year month)", () => {
  ...
  const testDates = ["0000-01", "9999-12", "2023-02", "2024-02", "2023-04"];

  const badTestDates = ["10000-02", "2023-13", "2023-16", "105-09", "20234-02"];
...
test("matchDatetime (ISO: YYYY-Www: year week)", () => {
  ...
  const testDates = ["0000-W01", "9999-W52", "2023-W01", "2024-W52", "2023-W04"];

  const badTestDates = ["10000-W02", "2023-W54", "2023-W56", "105-W09", "20234-W02"];
...
test("matchDatetime (ISO: YYYYWww: year week)", () => {
  ...
  const testDates = ["0000W01", "9999W52", "2023W01", "2024W52", "2023W04"];

  const badTestDates = ["10000W02", "2023W54", "2023W56", "105W09", "20234W02"];
...
test("matchDatetime (ISO: YYYY-DDD: Ordinal date (day number from the year))", () => {
  const pattern =
    descriptionToFormatCodeDate["ISO: YYYY-DDD: Ordinal date (day number from the year)"];
  const testDates = ["0000-001", "9999-365", "2023-001", "2024-366", "2023-120"];

  const badTestDates = ["10000-002", "2023-367", "2023-0366", "105-009", "20234-002"];
...
test("matchDatetime (ISO: YYYYDDD: Ordinal date (day number from the year))", () => {
  const pattern =
    descriptionToFormatCodeDate["ISO: YYYYDDD: Ordinal date (day number from the year)"];
  const testDates = ["0000001", "9999365", "2023001", "2024366", "2023120"];

  const badTestDates = ["10000021", "2023367", "20230366", "10500093", "20234002"];
...
test("matchDatetime (ISO: YYYY: year)", () => {
  ...
  const testYears = ["0000", "9999", "2023", "2024", "2023"];

  const badTestYears = ["10000", "20234", "2023-", "105", "20234-"];
...
test("matchDatetime (ISO: MM: month)", () => {
  ...
  const testMonths = ["01", "12", "02", "04", "09"];

  const badTestMonths = ["00", "13", "16", "13", "34"];
...
test("matchDatetime (ISO: DD: day)", () => {
  ...
  const testDays = ["01", "31", "29", "29", "31"];

  const badTestDays = ["001", "32", "35", "38", "000"];
...
test("matchDatetime (ISO: PnYnMnDTnHnMnS: duration)", () => {
  const pattern =
    descriptionToFormatCodeDate["ISO: PnYnMnDTnHnMnS :durations e.g. P3Y6M4DT12H30M5S"];
  const testDurations = [
    "P1Y",
    "P1M",
    "P1D",
...
test("matchDatetime (ISO: HH:MM: hour, minutes in 24 hour notation)", () => {
  const pattern =
    descriptionToFormatCodeDate["ISO: HH:MM: hour, minutes in 24 hour notation"];
  const testTimes = ["00:00", "23:59", "02:59", "04:59", "09:59"];

  const badTestTimes = ["00:60", "24:00", "25:00", "114:59", "00:61"];
...
test("matchDatetime (ISO: HH:MM:SS: hour, minutes, seconds in 24 hour notation)", () => {
  ...
  const testTimes = ["00:00:00", "23:59:59", "02:59:59", "04:59:59", "09:59:59"];
...
test("matchDatetime (DD/MM/YY: day, month, year)", () => {
  ...
  const testDates = ["01/01/00", "31/12/99", "29/02/23", "29/02/24", "31/04/23"];

  const badTestDates = ["32/02/100", "31/13/23", "31/16/23", "33/09/05", "30/02/234"];
...
test("matchDatetime (DDMMYYYY: day, month, year)", () => {
  ...
  const testDates = ["01010000", "31129999", "29022023", "29022024", "31042023"];

  const badTestDates = ["320210000", "31312023", "31612023", "310920105", "300220234"];
...
test("matchDatetime (MMDDYYYY: month, day, year)", () => {
  ...
  const testDates = ["01010000", "12319999", "02292023", "02292024", "04312023"];

  const badTestDates = ["0213201000", "31132023", "31162023", "0931105", "022300234"];
...
test("matchDatetime (H:MM or HH:MM: hour, minutes AM/PM)", () => {
  ...
  const testTimes = ["1:00 AM", "12:59 PM", "2:59 am", "4:59 pm", "9:59 AM"];

  const badTestTimes = ["00:60 am", "13:00 PM", "25:00 pm", "14:59 PM", "00:61 AM"];
