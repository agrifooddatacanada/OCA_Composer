...
import { transformToPackage, translateLinkMLToOCA } from '../src/SchemaTranslator/linkMLToOCA';
...
describe('YAML handler with sample LinkML file', () => {
  ...
  test('processes the LinkML schema through the conversion pipeline', () => {
    ...
    // Process through the pipeline with real implementations
    validateForOCATranslation(parsedYaml);
    const bundle = mapLinkMLToOCABundle(parsedYaml);
  ...
  test('identifies flagged attributes correctly', () => {
    // Parse YAML and process with real implementation
    const parsedYaml = yaml.load(yamlContent);
    const bundle = mapLinkMLToOCABundle(parsedYaml);
    
    // Check that flagged attributes are identified correctly
  ...
  test('includes unit information in the OCA bundle', async () => {
    // Translate the YAML to an OCA bundle using the real implementation
  ...
  test('verifies information overlay contains descriptions', async () => {
  ...
  test('verifies label overlay contains titles', async () => {
    // Translate the YAML to an OCA bundle
    const ocaBundle = await translateLinkMLToOCA(yamlContent);
  ...
  test('correctly maps LinkML data types to OCA types', async () => {
    // Translate the YAML to an OCA bundle
    const ocaBundle = await translateLinkMLToOCA(yamlContent);
    
    // Check data types in capture_base
    const attributes = ocaBundle.capture_base.attributes;
